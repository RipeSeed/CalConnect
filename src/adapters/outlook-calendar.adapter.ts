...
export class OutlookCalendarAdapter extends CalendarAdapterBase {
  ...
  constructor(
    ...
    refreshInterval: string = '55 minute',
  ) {
    super();
  ...
  async access(code: string, userId: string): Promise<any> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to get access token. Status: ${response.status}`,
        );
      }
  ...
  async getEventsInRange(
    ...
    try {
      ...
      const { outlook: outlookToken } = token;
      ...
      this.client = Client.init({
        authProvider: (done) => {
          done(null, outlookToken.accessToken);
        },
      });

      const timezoneHandledStart: string = adjustTimeByTimezone(
        startDate,
        timezone,
      );
      const timezoneHandledEnd: string = adjustTimeByTimezone(
        endDate,
        timezone,
      );
      ...
      const events = await this.client
        ...
        .filter(
          `start/dateTime ge '${timezoneHandledStart}' and end/dateTime le '${timezoneHandledEnd}'`,
        )
  ...
  async refreshAccessToken(userId: string): Promise<ICredentials> {
    try {
      ...
      if (!outlookToken.refreshToken) {
        throw new Error(
          'Refresh token is missing. Unable to refresh access outlookToken.',
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to refresh access outlookToken. Status: ${response.status}`,
        );
      }
      ...
      return {
        ...
        refreshToken: data.refresh_token || outlookToken.refreshToken,
      };
    } catch (error) {
